...
<body>
    <nav class="navbar fixed-top navbar-expand-lg navbar-dark bg-real">
        <div class="container">
            <a class="navbar-brand" href="{{ route('welcome') }}"><img src="{{ asset('logo/logo.png') }}" width="48"
                    alt="" srcset=""></a>
            @if (auth()->check())
    ...
    <section id="home" class="mt-3 py-5">
        <div class="container mt-5 py-5">
            <div class="row">
                <div class="col-md-8">
                    <img src="{{ asset('img/artikel/' . $artikel->img) }}" class="img-fluid w-100" alt=""
                        srcset="">
                    ...
                    @forelse($lains as $lain)
                        <div class="card mb-3" style="max-width: 540px;">
                            <div class="row no-gutters">
                                <div class="col-lg-5">
                                    <img src="{{ asset('img/artikel/' . $lain->img) }}" alt=""
                                        class="img-fluid h-100">
                                </div>
                                <div class="col-lg-7">
                                    <div class="card-body">
                                        <h5
                                            style="font-size: 12px; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; text-overflow: ellipsis;">
                                            {{ $lain->judul }}
                                        </h5>
                                        <p
                                        ...
                                        <div class="d-flex align-items-center">
                                            @if (trim($lain->isi) == '' || $lain->isi === null)
                                                <p class="mb-0 mr-2"><a href="{{ $lain->link }}" target="_blank"
                                                        class="btn btn-sm btn-warning" style="font-size: 10px">Read More
                                                        <span class="ion-ios-arrow-round-forward"></span></a></p>
                                            @else
                                                <p class="mb-0 mr-2"><a
                                                        href="{{ route('detail_artikel', ['id' => $lain->id]) }}"
                                                        class="btn btn-sm btn-warning" style="font-size: 10px">Read More
                                                        <span class="ion-ios-arrow-round-forward"></span></a></p>
                                            @endif
                                            <span
                                                style="font-size: 12px">{{ \Carbon\Carbon::parse($lain->updated_at)->format('j M Y') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
